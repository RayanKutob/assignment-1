...
function filterPlayers() {
    ...
    currentData = data.filter(p => {
        ...
        const matchTeam = team === 'all' || p.team.includes(team);
...
function sortBy(key, isTeam = false) {
    sortOrder[key] = sortOrder[key] === 'asc' ? 'desc' : 'asc';
    
    currentData.sort((a, b) => {
        let valA = isTeam ? a.team.match(/\(([^)]+)\)/)[1] : a[key];
        let valB = isTeam ? b.team.match(/\(([^)]+)\)/)[1] : b[key];
        
        if (typeof valA === 'number') {
            return sortOrder[key] === 'asc' ? valA - valB : valB - valA;
        }
        return sortOrder[key] === 'asc' ? valA.localeCompare(valB) : valB.localeCompare(valA);
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('th').forEach((th, i) => {
        th.addEventListener('click', () => {
            const cols = ['name', 'team', 'points', 'rebounds', 'assists'];
            sortBy(cols[i], i === 1);
        });
    });
});
